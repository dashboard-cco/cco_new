...
(function aplicarDesignModernoGraficosColuna() {
  ...
  const PALETAS = [
    ['rgba(14, 22, 245, 0.92)', 'rgba(2, 10, 56, 0.95)'],
  ...
  try {
    ...
    */
    ...
    if (salvouBanco) {
      ...
      alert("Planilha importada na tela, mas não foi salva no Supabase. Verifique Console e permissões do banco.");
    }
...
async function salvarBaseCompletaSupabase(nomeArquivo) {
  ...
  try {
    ...
    for (const nomeAba of Object.keys(sheetsOriginais)) {
      ...
      const lotes = dividirArrayEmLotes(linhas);
...
async function salvarBaseCompletaSupabase(nomeArquivo) {
  ...
  try {
    ...
    for (const nomeAba of Object.keys(sheetsOriginais || {})) {
      const aba = sheetsOriginais[nomeAba];
      const linhas = aba.dadosOriginais || [];
      const lotes = dividirEmLotesSupabase(linhas);
  ...
  try {
    ...
    for (const nomeAba of Object.keys(sheetsOriginais || {})) {
      ...
      const lotes = typeof dividirEmLotesSupabase === "function"
        ? dividirEmLotesSupabase(dadosAba, 500)
        : [dadosAba];
...
(function ccoV12FiltroMesRealSemLoop(){
  ...
  async function obterImportacaoPrincipal(){
    if (window.__CCO_IMPORTACAO_ATIVA__?.id) return window.__CCO_IMPORTACAO_ATIVA__;
    ...
    const { data, error } = await s.from('importacoes')
      ...
      .limit(1);
    if (error) throw error;
    const imp = (data || [])[0] || null;
    window.__CCO_IMPORTACAO_ATIVA__ = imp;
    return imp;
  }

  async function obterLimitesDatas(imp){
    const s = db(); if (!s || !imp?.id) return null;
    const q = 'data_operacao';
    const { data: primeira, error: e1 } = await s.from('operacoes').select(q).eq('importacao_id', imp.id).order('data_operacao', { ascending:true }).limit(1);
    if (e1) throw e1;
    const { data: ultima, error: e2 } = await s.from('operacoes').select(q).eq('importacao_id', imp.id).order('data_operacao', { ascending:false }).limit(1);
    if (e2) throw e2;
    const min = dataISO(primeira?.[0]?.data_operacao);
    const max = dataISO(ultima?.[0]?.data_operacao);
    if (!min || !max) return null;
    return { min, max };
  }

  function gerarPeriodosEntre(minISO, maxISO){
    const [a1,m1] = minISO.slice(0,7).split('-').map(Number);
    const [a2,m2] = maxISO.slice(0,7).split('-').map(Number);
    const out = [];
    let d = new Date(a1, m1-1, 1);
    const fim = new Date(a2, m2-1, 1);
    while (d <= fim) {
      out.push({ ano:String(d.getFullYear()), mes:pad(d.getMonth()+1), periodo:periodoKey(d.getFullYear(), d.getMonth()+1) });
  ...
  async function garantirPeriodos(){
    ...
    const imp = await obterImportacaoPrincipal();
    if (!imp?.id) return [];
    const limites = await obterLimitesDatas(imp);
    let periodos = limites ? gerarPeriodosEntre(limites.min, limites.max) : [];
    if (!periodos.length && imp.ano && imp.mes) periodos = [{ ano:String(imp.ano), mes:pad(imp.mes), periodo:periodoKey(imp.ano, imp.mes) }];
